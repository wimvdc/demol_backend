...
exports.getAllGroups = (userUuid) => {
  ...
  return db.executeQuery(
    `SELECT 'Publieke groep' name, (SELECT COUNT(*) FROM users) members, 'public' uuid
                            UNION
                            SELECT name, 
                            (SELECT COUNT(id) FROM users_in_group u WHERE uuid = group_uuid) members, 
                            (SELECT group_uuid FROM users_in_group WHERE uuid = group_uuid and user_uuid = ?) uuid
                            FROM groupz ORDER BY members desc;`,
    [userUuid]
  );
};
...
exports.getGroupByUuid = (uuid) => {
  return db.executeQuery(
    `SELECT name, uuid, share_code FROM groupz 
    WHERE uuid = ?;`,
    [uuid]
  );
};
...
exports.getUsersInGroup = (uuid) => {
  return db.executeQuery(
    `SELECT IF(ISNULL(nickname), CONCAT(firstName," ",UPPER(LEFT (lastName, 1))), nickname)  nickname, available_points availablepoints FROM users_in_group uig, users u
...
exports.getAllPublicUsers = () => {
  return db.executeQuery(`SELECT IF(ISNULL(nickname), CONCAT(firstName," ",UPPER(LEFT (lastName, 1))), nickname)  nickname, 
    ...
    FROM users u WHERE is_public = 1
    ORDER BY available_points desc;`);
};

exports.getMyGroups = (userUuid) => {
  return db.executeQuery(
    `SELECT name, uuid FROM groupz WHERE uuid in (
