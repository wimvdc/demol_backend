  ...
  try {
    ...
    for (let user of users) {
      ...
      guesses.forEach((guess) => {
        ...
        if (losers.includes(guess.mol_uuid)) lost += guess.points;
        else won += guess.points;
      });
      let newscore = won * 2 + (spendable - totalguessed) - lost;
      newscore = newscore <= 0 ? 3 : newscore + 3;
      console.info(`${user.uuid} now has ${newscore}`);
...
router.get("/calculate/endgametemp", isAdminLoggedIn, async (req, res, next) => {
  try {
    ...
    for (let user of users) {
      ...
      console.info(`${user.uuid} now has ${score} extra`);
